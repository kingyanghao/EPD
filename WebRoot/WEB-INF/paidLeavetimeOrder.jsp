...
%>	
	...
	<div id="main">
	<div id="lantiao" style="display: flex;flex-direction:column;justify-content:center"><div style="color: white;font-size:1.2vw;margin-left:2%;">Leave statistic</div></div>
			<div class="">
	...
	function getmonth(){
			...
			$.ajax({
			type: "POST",
			url: '<%=basePath%>main/paidLeaveSelectopeter.do',
	    	data: {
	    		val:val,
	    	},
			dataType:'json',
			...
			success: function(data){
				...
				if(data.succ==1){
						...
						for(var i=0;i<data.list3.length;i++){
							...
							html8.push('</div>');
							html8.push('<div class="s-ctn jianbian1" style="height:60px;margin-left:210px;">');
							 for(var n=0;n<data.list3[i].paid_leaveList.length;n++){
								html8.push('<div style="width:40px;height:60px;font-weight: bold;margin-right: 10px;text-align: center;line-height:60px;">'+data.list3[i].paid_leaveList[n].harmonious+'</div>');
							} 
							html8.push('</div></div>');
						} 
