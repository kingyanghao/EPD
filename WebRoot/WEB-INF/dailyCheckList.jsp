					...
					<!-- 右侧主体 左上角按钮 -->
								 ...
								 <div class="form-group">
									...
									<div class="layui-inline" style="border: solid 1px #cccccc;    border-radius: 4px;">
										<div class="layui-input-inline">
										   <input type="text" class="layui-input" id="test6" name="time" placeholder="请选择日期">
										</div>
									</div>
						...
						<div class="pull-right" style="margin-bottom:30px;">
							...
							style="border-radius:20px;font-size:14px;width:100px;"	onclick="deleteAll(this)"> 批量删除
							 ...
							 <a href="<%=basePath%>main/dailyCheck.do"><button class="btn btn-default btn-sm active"  data-toggle='batch-remove' 
							style="border-radius:20px;font-size:14px;width:100px;"	>点检
							</button></a>
							<a href="<%=basePath%>main/checkList.do"><button class="btn btn-default btn-sm active"  data-toggle='batch-remove' 
							style="border-radius:20px;font-size:14px;width:100px;"	>漏检
							</button></a>
		       			</div>
...
<!-- 跳转隐藏form表单 -->
	<form action="" method="post" id="jump" hidden="hidden">
		<input type="text" id="usernameId" name="daily_id" value ="" />
	</form>
	<form action="" method="post" id="jump1" hidden="hidden">
		<input type="text" id="usernameId1" name="daily_id" value ="" />
	</form>
</body>
	...
	$(function () {
		...
		function search(){
			//if($("#storeIdVal").val() != ""){
				...
				$.ajax({
					type : "POST",
					url : "<%=basePath%>main/dailyList.do",
						...
						if(result.success==-2){
							...
							//再添加
						 	$.each(result.daily_checkList, function(i, li) {
								var trs1 = '<tr>'
									+'<td style="width: 5%;"></td>'
									+ '<td style="width: 15%;"><input type="checkbox" name="box" lay-skin="primary" class="newsIdDel" id="'+li.daily_id+'"/></td>'
									+ '<td style="cursor:pointer;width: 15%;">'+ li.bench_name+'</td>'
									+ '<td style="width: 15%;">'+ li.implementer +'</td>'
									+ '<td style="width: 25%;">'+date(li.check_time)+'</td>'
									+ '<td style="width: 35%;" >'
									+ '<button class="layui-btn layui-btn-sm" onclick="updatedeails('+ li.daily_id +')"><i class="layui-icon layui-icon-search"></i></button>'
									+' <button class="layui-btn layui-btn-sm" onclick="deails('+ li.daily_id +')"><i class="layui-icon layui-icon-edit"></i></button>'
									+ '</td>'
									+ '</tr>'
								$("tbody").append(trs1);
							}); 
		...
		function deails(usernameId) {
			...
			$('#jump').attr("action", "<%=basePath%>main/dailyCheckDeails.do");
			$('#jump').submit();
		...
		function updatedeails(usernameId) {
			...
			$('#jump1').attr("action", "<%=basePath%>main/updateDailyCheckDeails.do");
			$('#jump1').submit();
