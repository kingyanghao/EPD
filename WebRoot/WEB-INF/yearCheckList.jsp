				...
				<div class="panel-body">
					...
					<!-- 右侧主体 左上角按钮 -->
						...
						<div class="pull-right" style="margin-bottom:30px;">
							...
							<a href="main/yearlyCheck.do"><button class="btn btn-default btn-sm active"  data-toggle='batch-remove' 
...
<!-- 跳转隐藏form表单 -->
	<form action="" method="post" id="jump" hidden="hidden">
		<input type="text" id="usernameId" name="year_id" value ="" />
	</form>
	
	<form action="" method="post" id="jump1" hidden="hidden">
		<input type="text" id="usernameId1" name="year_id" value ="" />
	</form>
	...
	$(function () {
		...
		function search(){
			//if($("#storeIdVal").val() != ""){
				...
				$.ajax({
					type : "POST",
					url : "<%=basePath%>main/yearlyList.do",
						...
						if(result.success==-2){
							...
							//再添加
						 	$.each(result.daily_checkList, function(i, li) {
								var trs1 = '<tr>'
									+'<td style="width: 5%;"></td>'
									+ '<td style="width: 15%;"><input type="checkbox" name="box" lay-skin="primary" class="newsIdDel" id="'+li.year_id+'"/></td>'
									+ '<td style="cursor:pointer;width: 15%;">'+ li.bench_name+'</td>'
									+ '<td style="width: 15%;">'+ li.username +'</td>'
									+ '<td style="width: 25%;">'+date(li.year)+'</td>'
									+ '<td style="width: 35%;" >'
									+ '<button class="layui-btn layui-btn-sm" onclick="updatedeails('+ li.year_id +')"><i class="layui-icon layui-icon-search"></i></button>'
									+' <button class="layui-btn layui-btn-sm" onclick="deails('+ li.year_id +')"><i class="layui-icon layui-icon-edit"></i></button>'
									+ '</td>'
									+ '</tr>'
								$("tbody").append(trs1);
								//待支付和退款中
		...
		function deleteAll(obj){
			...
			layer.alert('确认要彻底删除数据吗?', {
				skin: 'layui-layer-red',btn: ['确认','取消'] ,icon:3,closeBtn:0
						...
						$.ajax({			
							url:"<%=basePath%>main/deleteAllYearCheckList.do",
							type:"POST",
							dataType:"JSON",
							data:{"ids":ids},
							success:function(result){
		...
		function deails(usernameId) {
			...
			$('#jump').attr("action", "<%=basePath%>main/yearCheckDeails.do");
			$('#jump').submit();
		...
		function updatedeails(usernameId) {
			...
			$('#jump1').attr("action", "<%=basePath%>main/updateYearlyCheck.do");
			$('#jump1').submit();
