		...
		<table class="table table-striped" style="margin-top: 30px;">
 		<thead>
			<tr>
				...
				<th style="width: 15%;">Preferred Operator</th>
...
<!-- 跳转隐藏form表单 -->
	<form action="" method="post" id="jump" hidden="hidden">
		<input type="text" id="usernameId" name="orderid" value ="" />
	...
	$(function () {
		...
		function search(){
			//if($("#storeIdVal").val() != ""){
				...
				$.ajax({
					type : "POST",
					url : "<%=basePath%>main/modifyOrderList.do",
						...
						if(result.success==-2){
							...
							//再添加
						 	 $.each(result.work_orderList, function(i, li) {
						 		if(li.order_placer==result.username){
						 			 var trs1 = '<tr>'
											+'<td style="width: 15%;cursor:pointer;" onclick="deails('+ li.orderid +',\''+li.type+'\')">'+ li.va_no +'</td>'
											+'<td style="width: 15%;"   id="'+li.orderid+'">'+ li.kb_no +'('+li.type+')</td>'
											+ '<td style="width: 15%;">'+date(li.create_time)+'</td>'
											...
											+ '<td style="width: 10%;" >'
											+ '<i class="layui-icon layui-icon-survey" style="font-size: 24px; color: #1E9FFF;" onclick="updatedeails('+ li.orderid +',\''+li.type+'\')"></i>'
											+ '</td>'
											+ '</tr>'
										$("tbody").append(trs1);
						 		}else{
						 			 var trs1 = '<tr>'
											...
											+'<td style="width: 15%;"  id="'+li.orderid+'">'+ li.kb_no +li.type+'</td>'
											+ '<td style="width: 15%;">'+date(li.create_time)+'</td>'
											+ '<td style="width: 15%;">'+date(li.deadline) +'</td>'
											+ '<td style="width: 15%;">'+ li.preferred_operator +'</td>'
											+ '<td style="width: 15%;">'+li.order_placer+'</td>'
											+ '<td style="width: 10%;" >'
											+ '<i class="layui-icon layui-icon-survey" style="cursor:pointer;font-size: 24px; color: #1E9FFF;" ></i>'
											+ '</td>'
											+ '</tr>'
										$("tbody").append(trs1);
						 		}
		...
		function deails(usernameId,type) {
			...
			$('#typeId').val(type);
			$('#jump').attr("action", "<%=basePath%>main/Deails.do");
			$('#jump').submit(); 
		...
		function date(date1){
			...
			var nowDate = year + "-" + month + "-" + day+" "+hours+":"+mins;
			return nowDate;
		}
	</script>
</html>